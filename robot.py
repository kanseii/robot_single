...
# 加载光源图片
class Light(object):
    def __init__(self,pos,src,id):
        self.image_light = pygame.image.load(src)
...
class Robot(object):
    ...
    def __init__(self, pos=[10.0, 10.0], velocity=[0, 0]):
        self.image_open = pygame.image.load("open.png")
        self.image_shut = pygame.image.load("shut.png")
        self.rect = self.image_open.get_rect()
        self.rect_shut = self.image_shut.get_rect()
        ...
        self.rect_shut = self.rect_shut.move(pos[0], pos[1])
        self.step = 1
    # 当前运动方向(单位向量)
    def direction(self):
    ...
    def move(self,lights):
        ...
        for light in lights: 
            ...
            #self.vision(light) and 
            # 在视野范围内 未到达光源

            if(self.vision(light) and distance >= 20):
                ...
                ann_vr,ann_vl = np.array(forward_propagate(np.matrix([sl,sr]),theta1,theta2))[0]
                self.vr = ann_vr
                self.vl = ann_vl
                ...
                self.rect_shut = self.rect_shut.move(self.velocity[0]*self.step, self.velocity[1]*self.step)

                    
            
            # self.vision(light) and
            # 在视野范围内 已经到达光源
            elif (self.vision(light) and distance < 20):
    ...
    # 小车旋转
    def rotate(self):
        self.image = pygame.transform.rotate(self.image_open, self.angle)
        # self.image_shut = pygame.transform.rotate(self.image_shut, self.angle)
        if math.fabs(self.angle) >= 360:
            self.angle -= 360
        
        self.rect = self.image.get_rect(center = self.rect.center)  # 中心矫正
        self.rect_shut = self.image_shut.get_rect(center = self.rect_shut.center)  # 中心矫正
...
def init():
    ...
    for j in range(0,1):
        ...
        if j == 0:
            # light = Light(light_pos,RED,10,j)
            light = Light(light_pos,"light.png",j)
        else:
            # light = Light(light_pos,YELLOW,10,j) 
