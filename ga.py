...
class Robot(object):
    ...
    def get_angles(self):
        ...
        arr1 =  np.mat([POS_LIGHT[0] - x1,POS_LIGHT[1] - y1])
        arr2 =  np.mat([POS_LIGHT[0] - x2,POS_LIGHT[1] - y2])
    ...
    def move(self,motor_value):
        self.pos = self.rect.center
        arr_light = np.array(np.array(POS_LIGHT)-np.array(self.pos))
...
class GA():
    ...
    def __init__(self,
        ...
        self.elite_rate = elite_rate                            # 精英占比
        self.crossover_and_mutation = crossover_and_mutation    # 交叉且变异占比
        self.crossover_only = crossover_only                    # 仅交叉占比
        self.mutation_only = mutation_only                      # 仅突变占比
        self.crossover_percent_genes = crossover_percent_genes  # 基因交叉百分比
        self.mutation_percent_genes = mutation_percent_genes    # 突变百分比
        self.generations = n_generations                        # 繁衍次数
        self.robots = robots                                    # 个体
        ...
        self.mutation_type = mutation_type                      # 突变类型
    ...
    def update(self):       
        ...
        for ann_params  in self.population:
            ...
            motor_value = np.array(forward_propagate(np.matrix([self.robots[i].get_angles()]),theta1,theta2))[0]  
            self.robots[i].move(motor_value)
            i+=1
    ...
    def get_fitness(self):
        ...
        for robot in self.robots:
            arr_light = np.array(np.array(POS_LIGHT)-np.array(robot.pos))
            # 机器人中心到光源距离
            distance = np.linalg.norm(arr_light)
            fitness.append(1000/distance+1)
...
if __name__ == "__main__":
    ...
    selection_type = "rank" # roulette rank random tournament
